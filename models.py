...
class pos_return(models.Model):
	...
	state = fields.Selection(selection=[('draft','Borrador'),('done','Confirmado'),('used','Consumido')],default="draft")
        ...
        origin_id = fields.Many2one('pos.order',domain="[('partner_id','=',partner_id)]", states={'done':[('readonly', True)], 'draft':[('readonly',False)], 'used':[('readonly',True)]})
        date = fields.Date('Fecha',default=date.today(),readonly=True)
	return_line = fields.One2many(comodel_name='pos.return.line',inverse_name='return_id', states={'done':[('readonly', True)], 'draft':[('readonly',False)], 'used':[('readonly',True)]})
	session_id = fields.Many2one('pos.session',domain=[('state','=','opened')],required=True, states={'done':[('readonly', True)], 'draft':[('readonly',False)], 'used':[('readonly',True)]})
	journal_id = fields.Many2one('account.journal',domain=[('journal_user','=',True)],required=True, states={'done':[('readonly', True)], 'draft':[('readonly',False)], 'used':[('readonly',True)]})
